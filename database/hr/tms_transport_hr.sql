-- TMS Transport — HR fields for fleet drivers, loaders, mechanics

ALTER TABLE hr_employees ADD COLUMN IF NOT EXISTS license_number VARCHAR(30);
ALTER TABLE hr_employees ADD COLUMN IF NOT EXISTS license_expiry DATE;
ALTER TABLE hr_employees ADD COLUMN IF NOT EXISTS assigned_vehicle_id VARCHAR(20);
...
ALTER TABLE payroll_entries ADD COLUMN IF NOT EXISTS tms_allowance DECIMAL(12,2) NOT NULL DEFAULT 0;

INSERT INTO payroll_settings (key, value, description) VALUES
    ('route_allowance_default', '1500', 'Default monthly route allowance for TMS drivers'),
    ('fuel_allowance_per_trip', '50', 'Fuel allowance per completed trip (₹)'),
    ('loading_unloading_allowance', '500', 'Loading/unloading allowance for loaders (₹/month)'),
    ('halting_allowance_per_trip', '100', 'Halting/demurrage share per trip (₹)'),
    ('driver_bhatta_per_day', '200', 'Daily bhatta (DA) for long-haul drivers on road'),
    ('tms_safety_bonus', '1000', 'Monthly safety bonus if no incidents (flat for eligible drivers)'),
    ('license_expiry_alert_days', '30', 'Days before license expiry to flag in HR')
ON CONFLICT (key) DO NOTHING;

-- Sync driver license from drivers master
UPDATE hr_employees e SET
    license_number = d.license,
    license_expiry = d.license_expiry::DATE,
    route_allowance = 1500,
    fuel_allowance = 0,
    driver_bhatta = 200
FROM drivers d
WHERE e.driver_id = d.id AND e.employee_type = 'Driver';

-- Drop prior employee SP signatures (employment_hr_sp) before transport-extended versions.
\ir upgrade_drop_employee_sp.sql

-- Employee list (must exist after upgrade_drop; get/save below are transport-extended)
...
-- Extended get/save with TMS transport fields
...
BEGIN
    IF p_id IS NULL THEN
        INSERT INTO hr_employees (
            employee_code, name, employee_type, employment_type, department_id, designation_id,
            ...
            p_employee_code, p_name, COALESCE(p_employee_type, 'Staff'), COALESCE(p_employment_type, 'Permanent'),
