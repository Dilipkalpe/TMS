-- TMS Transport payroll: route/fuel/loading/bhatta allowances in generation
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM payroll_runs WHERE pay_month = p_month AND pay_year = p_year AND status IN ('Draft','Processed','Paid'))
    ...
    LOOP
        ...
        IF v_emp.employee_type = 'Driver' AND v_emp.driver_id IS NOT NULL THEN
            ...
            v_tms_allow := COALESCE(NULLIF(v_emp.route_allowance, 0), v_route_def)
                + (v_driver_trips * v_fuel_per_trip)
                + COALESCE(v_emp.fuel_allowance, 0)
...
RETURNS TABLE (
    ...
    other_allowance DECIMAL, tms_allowance DECIMAL, gross_pay DECIMAL, pf_deduction DECIMAL,
    advance_recovery DECIMAL, esi_deduction DECIMAL, insurance_deduction DECIMAL,
    other_deduction DECIMAL, days_worked DECIMAL, net_pay DECIMAL, payment_status VARCHAR, paid_at TIMESTAMPTZ
) AS $$
BEGIN
    RETURN QUERY SELECT e.id, e.run_id, e.employee_type, e.employee_id, e.employee_name,
        COALESCE(e.employment_type, 'Permanent'), e.basic_salary, e.trip_incentive, e.overtime,
        e.other_allowance, COALESCE(e.tms_allowance, 0), e.gross_pay, e.pf_deduction, e.advance_recovery,
        e.esi_deduction, e.insurance_deduction, e.other_deduction, e.days_worked, e.net_pay,
        e.payment_status, e.paid_at
    FROM payroll_entries e WHERE e.run_id = p_run_id ORDER BY e.employee_name;
END;
$$ LANGUAGE plpgsql;
...
RETURNS TABLE (
    ...
    other_allowance DECIMAL, tms_allowance DECIMAL, gross_pay DECIMAL,
...
BEGIN
    RETURN QUERY SELECT pe.id, pr.id, pr.run_code, pr.period_label, pr.pay_month, pr.pay_year, pr.status,
        pe.employee_type, pe.employee_id, pe.employee_name, COALESCE(pe.employment_type, 'Permanent'),
        d.name, g.name, pe.basic_salary, pe.trip_incentive, pe.overtime, pe.other_allowance,
        COALESCE(pe.tms_allowance, 0), pe.gross_pay, pe.pf_deduction, pe.esi_deduction, pe.insurance_deduction,
        pe.advance_recovery, pe.other_deduction, pe.days_worked, pe.net_pay, pe.payment_status, pe.paid_at,
        COALESCE(cs.company_name, 'TMS Pro Transport')::VARCHAR
