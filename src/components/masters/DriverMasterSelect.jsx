...
function displayLines(row) {
  ...
  const bits = [row?.phone, row?.license].filter(Boolean)
...
export default function DriverMasterSelect({
  label,
  displayValue = '',
  onSelect,
  ...
  const masterConfig = useMemo(() => getLookupMasterConfig('drivers'), [])
  const entityLabel = masterConfig?.entityLabel ?? 'Driver'
  ...
  const popupId = useRef(`driver-master-${Math.random().toString(36).slice(2)}`).current
