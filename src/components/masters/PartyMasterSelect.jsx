...
import { buildLookupCountText, partyDisplayLines } from '../../utils/lookupDropdownUtils'
...
import { getLookupMasterConfig, resolvePartyMasterKey } from '../../config/lookupMasterConfig'

const PARTY_PAGE_SIZE = 25

function formatLabel(row) {
  const { primary, secondary } = partyDisplayLines(row)
...
export default function PartyMasterSelect({
  ...
  const resolvedMasterKey = masterKeyProp ?? resolvePartyMasterKey(api)
  const masterConfig = useMemo(
    () => (resolvedMasterKey ? getLookupMasterConfig(resolvedMasterKey) : null),
    [resolvedMasterKey],
  )
  const entityLabel = masterConfig?.entityLabel ?? 'Record'
  ...
  const popupId = useRef(`party-master-${Math.random().toString(36).slice(2)}`).current
  ...
  return (
    <>
      <div ref={wrapRef} className={`relative ${className}`}>
        <OutlinedField label={label} fieldClassName={fieldClass}>
          <input
            ref={inputRef}
            type="text"
            value={query}
...
export { formatLabel as formatPartyLabel }
