...
function itemDisplayLines(row) {
  const primary = row?.name ?? ''
  const bits = [row?.hsn ? `HSN ${row.hsn}` : null, row?.defaultPackageType || null].filter(Boolean)
...
export default function ItemMasterSelect({
  label,
  valueId = '',
  displayValue = '',
  onSelect,
  ...
  const masterConfig = useMemo(() => getLookupMasterConfig('items'), [])
  const entityLabel = masterConfig?.entityLabel ?? 'Item'
  ...
  const controlClass = isDense ? 'outlined-control outlined-control--dense lr-entry-v2-cell-input' : OUTLINED_CONTROL_CLASS
