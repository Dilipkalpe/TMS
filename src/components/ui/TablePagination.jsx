  ...
  return (
    <div className="flex shrink-0 flex-col gap-2 border border-t-0 border-primary/20 bg-slate-50 px-2 py-2 sm:flex-row sm:items-center sm:justify-between sm:px-3 dark:bg-slate-800/50">
      <p className="text-xs text-slate-600 sm:text-sm dark:text-slate-400">
        ...
        <span className="hidden sm:inline">{totalRecords.toLocaleString('en-IN')} records</span>
        <span className="mx-2 hidden sm:inline">•</span>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <button type="button" className={btnClass} disabled={page >= totalPages} onClick={() => onPageChange(page + 1)}>
            <ChevronRight className="h-4 w-4" />
          </button>
          <button type="button" className={btnClass} disabled={page >= totalPages} onClick={() => onPageChange(totalPages)}>
