...
export default function TablePagination({
  ...
  return (
    <div className="flex shrink-0 flex-col gap-1.5 border border-t-0 border-primary/20 bg-slate-50 px-2 py-1.5 sm:flex-row sm:items-center sm:justify-between sm:px-3 dark:bg-slate-800/50">
      <p className="text-xs text-slate-600 sm:text-sm dark:text-slate-400">
        {showingFrom != null && showingTo != null ? (
          <>
            ...
            <span className="font-semibold">{totalIsApproximate ? '~' : ''}{totalRecords.toLocaleString('en-IN')}</span> entries
          </>
        ) : (
          <>
