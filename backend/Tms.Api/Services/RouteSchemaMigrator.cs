...
public static class RouteSchemaMigrator
{
    ...
    static async Task<string> LoadSchemaSqlAsync(CancellationToken ct)
    {
        ...
        throw new FileNotFoundException("database/routing/schema.sql not found");
    }

    static IEnumerable<string> SchemaPathCandidates()
    {
        yield return Path.Combine(AppContext.BaseDirectory, "database", "routing", "schema.sql");
        yield return Path.Combine(Directory.GetCurrentDirectory(), "database", "routing", "schema.sql");
        yield return Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "database", "routing", "schema.sql"));
    }
