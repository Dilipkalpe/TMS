...
public static class EnterpriseModuleSchemaMigrator
{
    public static async Task EnsureAsync(TmsDbContext db, CancellationToken ct = default)
    {
        ...
        await SeedDemoDataAsync(conn, ct);
    }
    ...
    {
        yield return Path.Combine(AppContext.BaseDirectory, "database", "modules", "schema.sql");
        yield return Path.Combine(Directory.GetCurrentDirectory(), "database", "modules", "schema.sql");
        yield return Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "database", "modules", "schema.sql"));
        yield return Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), "..", "..", "database", "modules", "schema.sql"));
    }
    ...
    static Task SeedDemoDataAsync(NpgsqlConnection conn, CancellationToken ct) => Task.CompletedTask;
